...
const generateHash = async (password) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "error generating salted hash of password" });
  }
};
...
const checkIfUsernameExists = async (req, res, next) => {
  try {
    if (req.body && req.body.username) {
      ...
      res.status(401).json({ message: "error: please provide a username" });
    }
  } catch (error) {
    throw error;
  }
    ...
    if (req.body) {
      ...
      if (!username || !email || !password) {
        res
          ...
          .json({
            message:
              "error: username, email, and password are required. New user registration failed!",
          });
      } else {
        ...
        res
          .status(201)
          .json({ message: `user created with user id: ${newUserId}` });
      }
    }
  } catch (error) {
...
router.post("/login", checkIfUsernameExists, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "server error ocurred while attempting to log in",
        error,
      });
  }
});
